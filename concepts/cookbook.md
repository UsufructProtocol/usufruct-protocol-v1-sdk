...
```ts
import { usufruct, SUI, coinTag } from '@usufruct-protocol/sdk';
...
// either fetch its metadata on-chain, or build the tag synchronously if you know
// the decimals/symbol:
//   const USDC = await u.coinType('0x…::usdc::USDC');                              // async, fetches metadata
//   const USDC = coinTag({ type: '0x…::usdc::USDC', decimals: 6, symbol: 'USDC' }); // sync, no round-trip
