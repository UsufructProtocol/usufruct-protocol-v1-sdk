/**
 * `apply_pending_transition_states` — the mirror (off-chain `step`), paired
 * with the core's `applyToPtb`. `step` mirrors
 * `asset_state::execute_apply_pending_transition_states`: step_handover →
 * step_tenure_expiry → step_auction_expiry, each firing only when its boundary
 * is crossed. The handover settlement is the one curve-driven transition; its
 * economic split comes from the deterministic curve math in `sim/curve.ts`.
 */
import {
  applyToPtb,
  type ApplyPtbArgs,
